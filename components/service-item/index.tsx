...
export default function ServiceItem({ service }: ServiceItemProps) {
  ...
  return (
    <Sheet open={sheetIsOpen} onOpenChange={setSheetIsOpen}>
      <div
        className={
          "border-border bg-card flex items-center justify-center gap-3 rounded-2xl border border-solid p-3"
        }
      >
        ...
        <div className={"flex grow basis-0 flex-row items-center self-stretch"}>
          <div
            className={"relative flex h-full min-h-0 min-w-0 grow basis-0 flex-col items-start justify-between"}
          >
            <div className={"flex h-[67.5px] w-full flex-col items-start gap-1 text-sm leading-[1.4]"}>
              <p className={"text-card-foreground w-full font-bold"}>{service.name}</p>
              <p className={"text-muted-foreground w-full font-normal"}>{service.description}</p>
            </div>
          ...
          {selectedDate && (
            <>
              ...
              <div className={"flex flex-col gap-3 px-5"}>
                <div
                  className={
                    "border-border bg-card flex w-full flex-col gap-3 rounded-[10px] border border-solid p-3"
                  }
                >
